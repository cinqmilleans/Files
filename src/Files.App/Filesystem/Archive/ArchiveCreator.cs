...
namespace Files.App.Filesystem.Archive
{
	public class ArchiveCreator : IArchiveCreator
	{
		...
		public async Task<bool> CreateArchive()
		{
			var archive = GetArchiveName();
			var sources = Sources.ToArray();

			var compressor = new SevenZipCompressor
			{
				ArchiveFormat = GetSevenZipArchiveFormat(),
				CompressionLevel = GetSevenZipCompressionLevel(),
				VolumeSize = GetSevenZipVolumeSize(),
		...
		private string GetArchiveName() => FileName + FileFormat switch
		{
			ArchiveFormats.Zip => ".zip",
			ArchiveFormats.SevenZip => ".7z",
			_ => throw new ArgumentOutOfRangeException(nameof(FileFormat)),
		};
		private OutArchiveFormat GetSevenZipArchiveFormat() => FileFormat switch
		{
			ArchiveFormats.Zip => OutArchiveFormat.Zip,
			ArchiveFormats.SevenZip => OutArchiveFormat.SevenZip,
			_ => throw new ArgumentOutOfRangeException(nameof(FileFormat)),
		};
		private CompressionLevel GetSevenZipCompressionLevel() => CompressionLevel switch
		{
		...
		private int GetSevenZipVolumeSize() => SplittingSize switch
		{
