	...
	public readonly struct HotKey : IEquatable<HotKey>
	{
		...
		public string Code
		{
			get
			{
				return (Key, Modifier) switch
				{
					...
					_ => $"{GetVisibleCode(IsVisible)}{Key}{separator}{GetModifierCode(Modifier)}",
				};

				static string GetVisibleCode(bool isVisible)
					=> isVisible ? string.Empty : $"{notVisible}";

				static string GetModifierCode(KeyModifiers modifiers)
				{
					StringBuilder builder = new();
					if (modifiers.HasFlag(KeyModifiers.Menu))
						builder.Append($"{separator}{KeyModifiers.Menu}");
					if (modifiers.HasFlag(KeyModifiers.Ctrl))
						builder.Append($"{separator}{KeyModifiers.Ctrl}");
					if (modifiers.HasFlag(KeyModifiers.Shift))
						builder.Append($"{separator}{KeyModifiers.Shift}");
					if (modifiers.HasFlag(KeyModifiers.Win))
						builder.Append($"{separator}{KeyModifiers.Win}");
					builder.Remove(0, 1);
		...
		public string Label
		{
			get
			{
				...
				{
					(Keys.None, KeyModifiers.None) => string.Empty,
					(Keys.None, _) => GetCode(Modifier),
					(_, KeyModifiers.None) => Key.ToString(),
					_ => $"{Key}{separator}{GetCode(Modifier)}",
				};

				static string GetCode(KeyModifiers modifier)
				{
					StringBuilder builder = new();
					if (modifier.HasFlag(KeyModifiers.Menu))
						builder.Append($"{separator}{modifiers[KeyModifiers.Menu]}");
					if (modifier.HasFlag(KeyModifiers.Ctrl))
						builder.Append($"{separator}{modifiers[KeyModifiers.Ctrl]}");
					if (modifier.HasFlag(KeyModifiers.Shift))
						builder.Append($"{separator}{modifiers[KeyModifiers.Shift]}");
					if (modifier.HasFlag(KeyModifiers.Win))
						builder.Append($"{separator}{modifiers[KeyModifiers.Win]}");
					builder.Remove(0, 1);
					return builder.ToString();
				}
			}
