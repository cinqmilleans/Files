...
public abstract class AbstractHelpers : IFilesystemHelpers
{
	...
	public AbstractHelpers(IShellPage associatedInstance, CancellationToken cancellationToken)
	{
	...
	public async Task<ReturnResult> CreateShortcutFromClipboard(DataPackageView packageView, string destination, bool showDialog, bool registerHistory)
	{
		...
		var dest = source.Select(x => Path.Combine(destination, AbstractHelpers.GetShortcutNamingPreference(x.Name)));
		source = await source.ToListAsync();
