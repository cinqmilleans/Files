...
namespace Files.Filesystem.Search
{
    internal class FolderSearch
    {
        ...
        public FolderSearchOptionCollection Options { get; } = new FolderSearchOptionCollection();
        ...
        public async Task<ObservableCollection<ListedItem>> SearchAsync()
        {
            var search = new Search
            {
                ...
                MaxItemCount = MaxItemCount > 0 ? MaxItemCount : uint.MaxValue,
        ...
        private class Search
        {
            ...
            private async Task<IList<ListedItem>> SearchAsync(StorageFolder folder)
            {
                ...
                var stepSize = Math.Min(defaultStepSize, MaxItemCount);
                ...
                while (items.Count > 0)
                {
                    ...
                    stepSize = Math.Min(defaultStepSize, MaxItemCount - (uint)results.Count);
            ...
            private async Task AddItemsAsync(string folder, ObservableCollection<ListedItem> results)
            {
                ...
                {
                    foreach (var item in await SearchWithWin32Async(folder, hiddenOnlyFromWin32, MaxItemCount - (uint)results.Count))
                    {
                        results.Add(item);
                    }
                }
